/*
All credits goes to Devoxin: https://github.com/Devoxin/JukeBot/tree/master/src/main/java/jukebot/audio/sourcemanagers/pornhub
Patched by Stonedestroyer for PH breakage.
 */
...
object Utils {
    ...
    fun extractMediaString(page: String): String {
        ...
        if(validFormats.isEmpty()) {
            throw IllegalStateException("Failed to parse valid formats from dictionary.")
        } else if (validFormats.size > 1) {
            throw IllegalStateException("Contains too many entries from parsed dictionary.")
        }
